"""Bridge engine CombatState <-> Sim 3 tuple DP."""
...
def fight_params_from_state(state: CombatState, *, max_turns: int = 30) -> FightParams:
    ...
    return FightParams(
        ...
        enemy_hp_loss_cap=enemy.hp_loss_cap_per_turn,
    )


def solve_optimal_sim3(
    state: CombatState,
    *,
    max_turns: int = 30,
    shuffle_seed: int = 0,
    ...
    return solve_with_best_play_sim3(st, params, shuffle_seed, memo=memo)[0]


@dataclass
class OptimalTurnDecisionSim3:
    action: TurnAction
    composition: TurnCompositionSim3
    value: DpValue
    candidates: list[tuple[TurnCompositionSim3, DpValue]] = field(default_factory=list)
...
def action_for_composition_sim3(
    state: CombatState,
    ...
    inflames = [c for c in state.hand if c.definition.card_id == "INFLAME"]
    ...
    for card in defends[: comp.defends]:
        plays.append((card.instance_id, None))
    for card in inflames[: comp.inflame]:
        plays.append((card.instance_id, None))
...
def choose_optimal_turn_sim3(
    ...
    *,
    ...
    memo: dict | None = None,
) -> OptimalTurnDecisionSim3:
    params = fight_params_from_state(state, max_turns=max_turns)
    st = state_from_engine(state, params.pattern)
    table: dict = memo if memo is not None else {}
    best_known = [LOSS_HP]

    value, best_comp = solve_with_best_play_sim3(
        st, params, shuffle_seed, memo=table, use_prunes=True, best_known=best_known
    )
    comp = best_comp or TurnCompositionSim3(0, 0, 0, 0, 0)

    candidates: list[tuple[TurnCompositionSim3, DpValue]] = []
    if list_candidates:
        for c in ordered_plays_sim3(st, params.pattern):
    ...
    return OptimalTurnDecisionSim3(
        action=action_for_composition_sim3(state, comp),
        composition=comp,
        value=value,
