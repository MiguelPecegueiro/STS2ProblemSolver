"""Bridge engine CombatState <-> Sim 1 tuple DP."""
...
from combat_sim.sim1.composition import TurnCompositionSim1
...
def solve_optimal_sim1(
    ...
    *,
    ...
    return solve_with_best_play_sim1(st, params, shuffle_seed, memo=memo)[0]


@dataclass
class OptimalTurnDecisionSim1:
    action: TurnAction
    composition: TurnCompositionSim1
    value: DpValue
    candidates: list[tuple[TurnCompositionSim1, DpValue]] = field(default_factory=list)
...
def action_for_composition_sim1(
    state: CombatState,
...
def choose_optimal_turn_sim1(
    ...
    *,
    ...
    memo: dict | None = None,
) -> OptimalTurnDecisionSim1:
    params = fight_params_from_state(state, max_turns=max_turns)
    st = state_from_engine(state, params.pattern)
    table: dict = memo if memo is not None else {}
    best_known = [LOSS_HP]

    value, best_comp = solve_with_best_play_sim1(
        st, params, shuffle_seed, memo=table, use_prunes=True, best_known=best_known
    )
    comp = best_comp or TurnCompositionSim1(0, 0, 0)

    candidates: list[tuple[TurnCompositionSim1, DpValue]] = []
    if list_candidates:
        for c in ordered_plays_sim1(st, params.pattern):
    ...
    return OptimalTurnDecisionSim1(
        action=action_for_composition_sim1(state, comp),
        composition=comp,
        value=value,
