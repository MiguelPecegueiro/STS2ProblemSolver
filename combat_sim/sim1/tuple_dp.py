"""Pure tuple DP for Sim 1 (Bash + Vulnerable)."""
...
def state_from_engine(state, pattern: Pattern) -> TupleStateSim1:
    from combat_sim.deck_counts import hand_counts_sim1

    enemy = state.living_enemies()[0] if state.living_enemies() else state.enemies[0]
    hc = hand_counts_sim1(state)
...
def _apply_player_plays(
    ...
    block_e: int,
    vuln_e: int,
    comp: TurnCompositionSim1,
    *,
    enemy_slow: bool = False,
) -> tuple[int, int, int]:
    """Play order: Defends (block only), Bash, Strikes — maximizes strike damage with vuln."""
    slow_n = 0
    if enemy_slow:
        slow_n += comp.defends
...
def apply_turn_sim1(
    ...
    *,
    ...
    if comp.strikes > st.hand_s or comp.defends > st.hand_d or comp.bash > st.hand_b:
        return None
    if comp.energy_cost > MAX_ENERGY:
        return None
    ...
    hp_e, block_e, vuln_e = _apply_player_plays(
        hp_e, block_e, vuln_e, comp, enemy_slow=enemy_slow
    )
...
def ordered_plays_sim1(st: TupleStateSim1, pattern: Pattern) -> list[TurnCompositionSim1]:
    plays = feasible_plays_sim1(st.hand_s, st.hand_d, st.hand_b)
    incoming = 0
    if pattern:
    ...
    filtered: list[TurnCompositionSim1] = []
    for comp in plays:
        if comp.cards_played == 0 and (st.hand_s > 0 or st.hand_b > 0) and st.hp_e > 0:
            continue
        ...
        if (
            comp.cards_played == 0
            and st.hp_e > 0
            and (st.hand_s > 0 or st.hand_b > 0 or (st.hand_d > 0 and incoming > 0))
        ):
            continue
        filtered.append(comp)

    # Sort for deterministic tie-breaks only (full search still runs).
    filtered.sort(key=lambda c: (-c.defends, -c.bash, -c.strikes, -c.cards_played))
    return filtered or [TurnCompositionSim1(0, 0, 0)]
...
def _block_max_per_turn(st: TupleStateSim1) -> int:
    return DEFEND_BLOCK * min(MAX_ENERGY, st.hand_d + st.draw.count("D"))


def _damage_min_survival(st: TupleStateSim1, params: FightParams) -> int:
    total = 0
...
def _damage_max_kill(st: TupleStateSim1, max_turns: int) -> int:
    ...
    per_turn = int(STRIKE_DAMAGE * min(MAX_ENERGY, s) * 1.5)
    if b > 0:
        per_turn += int(damage_with_vulnerable(BASH_DAMAGE, 1) * min(1, b))
    return per_turn * turns_left


def _hp_ceiling(st: TupleStateSim1, params: FightParams) -> int:
    return st.hp_p - _damage_min_survival(st, params)


def _apply_forward_prunes(
    st: TupleStateSim1,
    params: FightParams,
) -> str | None:
    """Absolute prunes only (safe to memo)."""
    if st.hp_e <= 0:
        return None
...
def _ceiling_prune(st: TupleStateSim1, params: FightParams, best_known_hp: int) -> bool:
    """Context-dependent: prune branch but never cache (see docs/COMBAT_SIM.md)."""
    if best_known_hp <= LOSS_HP:
        return False
    return _hp_ceiling(st, params) < best_known_hp


MemoEntry = tuple[DpValue, TurnCompositionSim1 | None]
    ...
    *,
    ...
    if use_prunes:
        ...
        if reason in ("survival", "kill"):
            entry: MemoEntry = (DpValue(LOSS_HP, 0), TurnCompositionSim1(0, 0, 0))
            memo[key] = entry
            return entry
    ...
    best_comp: TurnCompositionSim1 | None = None
    for comp in ordered_plays_sim1(st, params.pattern):
    ...
    if best_comp is None and best.winnable:
        best_comp = TurnCompositionSim1(0, 0, 0)
